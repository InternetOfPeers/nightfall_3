...
// import logger from 'common-files/utils/logger.mjs'; //can't use the logger - one of the dependencies depends on async_hooks - can't be browserified
...
function createQueue(options) {
  ...
  queue.on('error', error => console.log({ msg: 'Error caught by queue', error }));
...
class Nf3 {
  ...
  applyHttpClientAuthentication() {
    if (!this.clientAuthenticationKey) {
      console.log('No client authentication key is set!');
      return;
    }
  ...
  */
  async submitTransaction(unsignedTransaction, contractAddress = this.shieldContractAddress, fee) {
    ...
    console.log(`Sending transaction with hash ${tx.transactionHash}`);
  ...
  */
  ...
  async deposit(
    ...
    return new Promise((resolve, reject) => {
      userQueue.push(async () => {
        try {
          console.log('Deposit transaction being processed');
  ...
  async getInstantWithdrawalRequestedEmitter() {
    ...
    connection.onopen = () => {
      ...
      console.log('Liquidity provider websocket connection opened');
    ...
    return new Promise((resolve, reject) => {
      proposerQueue.push(async () => {
        try {
          ...
          console.log('Proposer registered on chain');
          resolve(receipt);
        } catch (err) {
          console.log(`Register proposer failed with error ${err.message}`);
          reject(err);
        }
      });
    });
  ...
  async updateProposer(url, stake, fee) {
    ...
    console.log(`Proposer with address ${this.ethereumAddress} updated to URL ${url}`);
    return new Promise((resolve, reject) => {
      proposerQueue.push(async () => {
        try {
          const receipt = await this.submitTransaction(
  ...
  createEmitter() {
     ...
     */
    emitter.on('error', error => console.log({ msg: 'Error caught by emitter', error }));
  ...
  async startProposer() {
    ...
    connection.onerror = () => console.log('Proposer websocket connection error');
    connection.onclosed = () => console.log('Proposer websocket connection closed');
  ...
  async sendOffchainTransaction(transaction) {
    ...
    console.log({ msg: 'Peer List', peerList });
    return Promise.any(
  ...
  async startChallenger() {
    ...
    connection.onopen = () => {
      ...
      // and a listener for the pong
      console.log('Challenge websocket connection opened');
    ...
    connection.onmessage = async message => {
      ...
      console.log(`Challenger received websocket message of type ${type}`);
      ...
      if (type === 'commit' || type === 'challenge') {
        ...
        console.log(`queued ${type} ${txDataToSign}`);
      }
      if (type === 'rollback') {
    ...
    connection.onerror = () => console.log('websocket connection error');
    connection.onclosed = () => console.log('websocket connection closed');
    ...
    return new Promise(resolve => {
      if (challengerQueue.autostart) {
        ...
        challengerQueue.unshift(async () => {
          ...
          console.log(`queue challengerQueue has been paused`);
          resolve();
        });
      } else {
        resolve();
  ...
  unpauseQueueChallenger() {
    ...
    challengerQueue.unshift(async () => console.log(`queue challengerQueue has been unpaused`));
  }
  ...
  */
  ...
  startPeriodicPayment(cronExp = '0 0 * * */6') {
    this.periodicPaymentJob = createJob(cronExp, async () => {
      try {
        console.log(`--in cron job --- ${new Date().toLocaleString()}`);
